...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"WARNING: Gagal inisialisasi GEE: {str(e)}")
...
async def analyze_land_satellite(payload: PolygonRequest):
    try:
        ...
        today = datetime.now()
        end_date = payload.end_date or today.strftime("%Y-%m-%d")
        # Dioptimasi menjadi 30 hari agar eksekusi cepat
        start_date = payload.start_date or (today - timedelta(days=30)).strftime("%Y-%m-%d")
...
async def get_all_lands_ndvi_tile(payload: MultiPolygonRequest):
    try:
        today = datetime.now()
        end_date = payload.end_date or today.strftime("%Y-%m-%d")
        # Dioptimasi menjadi 30 hari untuk mencegah HTTP 502 Timeout
        start_date = payload.start_date or (today - timedelta(days=30)).strftime("%Y-%m-%d")

        # Parsing sanitasi array poligon
        clean_polygons = []
        ...
        if not clean_polygons:
            raise HTTPException(status_code=400, detail="Data poligon kosong.")

        map_tile_result = await asyncio.to_thread(
            get_all_lands_ndvi_map_tile,
            polygons_coordinates=clean_polygons,
            start_date=start_date,
            end_date=end_date
        )
        ...
        if not map_tile_result:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Citra satelit bebas awan tidak ditemukan untuk rentang tanggal ini."
            )
