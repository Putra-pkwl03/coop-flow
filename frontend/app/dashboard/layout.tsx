...
export default function AdminKoperasiLayout({ children }: { children: React.ReactNode }) {
  ...
  const [isOnline, setIsOnline] = useState<boolean>(true); // 🌟 State status jaringan
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      // Set status awal & event listener koneksi
  ...
  return (
    <div className="flex min-h-screen bg-[#f8fafc] text-zinc-800 antialiased font-sans">
      {/* 🌟 HANYA RENDER SCRIPT JIKA ONLINE! Mencegah DNS error saat offline */}
      {isOnline && (
        <Script
          ...
          strategy="lazyOnload"
          onError={() => {
            console.warn("UserWay Widget gagal dimuat.");
          }}
        />
      )}
