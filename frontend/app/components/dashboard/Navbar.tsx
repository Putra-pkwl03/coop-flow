...
// Peta role -> path halaman profil.
// Role yang tidak terdaftar di sini otomatis jatuh ke pola default
// `/dashboard/{role}/profil` (lihat getProfilePath di bawah).
...
export default function Navbar({
  ...
  // Path halaman profil, mengikuti role yang sedang login
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-5">
        ...
        >
          <FaBell
            className={`text-xl ${isMandatoryFullLayout ? "text-white" : "text-slate-600"}`}
          />
          <span
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <div className="text-left hidden sm:block">
              <p
                className={`text-sm font-bold leading-tight ${isMandatoryFullLayout ? "text-white" : "text-slate-800"}`}
              >
                {adminName}
              </p>
              <p
                className={`text-[11px] font-medium mt-0.5 uppercase tracking-wide ${isMandatoryFullLayout ? "text-emerald-200" : "text-slate-400"}`}
              >
                {roleName}
              </p>
            </div>
            <FaChevronDown
              className={`text-xs transition ${isMandatoryFullLayout ? "text-emerald-200 group-hover:text-white" : "text-slate-400 group-hover:text-slate-600"}`}
            />
          </div>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-3 w-48 bg-white rounded-2xl shadow-xl border border-zinc-100/80 py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              {/* Tombol Menuju Halaman Profil (path mengikuti role yang sedang login) */}
              <Link
                href={profilePath}
